...
4. **An undergraduate course** — `course/`: *Tight-Binding Physics with PythTB*,
   ten lectures as offline reveal.js slides (`course/deck/index.html`), an A4
   handout (`course/handout/handout.html`) and lecturer notes
   (`course/notes/LECTURER_NOTES.md`). Every figure on the slides is a figure
   produced by a notebook cell, shown with the notebook's caption, figure number
   and cell. Each lecture is a vertical stack of slides ordered intro → core →
   math; see `course/README.md` for the syllabus and the presenting keys.
...
```bash
python course/tools/extract_figures.py   # notebook PNG outputs -> course/deck/figs + provenance.json; --notebook F, --outdir D, --check, --quiet
...
every figure shown byte-identical to the notebook output with its provenance, the
generated deck/handout/notes up to date, every text key in `index.html` resolving,
...
- The course (`course/`) shows only figures generated by notebook cells — `provenance.json` maps each one to its section, cell, figure number and caption, and the suite fails if a figure on disk no longer matches the notebook output.
